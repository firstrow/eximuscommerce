...
abstract class CActiveRecord extends CModel
{
	...
	public function getRelated($name,$refresh=false,$params=array())
	{
		...
		if($params!==array())
		{
			...
			$r = $this->_related[$name];

		// Added by firstrow@gmail.com
		// afterFind() - doesnt work for relations with `through`
		// Quick fix.
		if(property_exists($relation, 'through') && $relation->through !== null && $relation instanceof CHasOneRelation && method_exists($r, 'afterFind'))
			$r->afterFind();
		elseif(property_exists($relation, 'through') && $relation->through !== null && $relation instanceof CHasManyRelation && !empty($r))
		{
			foreach($r as $model)
				$model->afterFind();
		}
